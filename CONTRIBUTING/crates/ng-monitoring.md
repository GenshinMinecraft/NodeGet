...
- **`get_or_insert` 会复活软删行**（`monitoring_uuid_cache.rs:207`）：对软删行设置 `soft_delete=false` 并更新内存 map；`soft_delete` 仅置标志，行仍留在 `by_uuid`/`by_id`。`list_all()` 过滤软删，但 `get_id`/`get_uuid` 仍解析软删 UUID——通过 `get_id` 检查「存在」的代码会把软删 agent 视为存在。

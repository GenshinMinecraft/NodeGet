...
- `crontab_task` (`task.rs:72`)：Phase 1 序列化 `task_event_type` 一次；Phase 2 构建 `Vec<task::ActiveModel>`（per-agent 随机 token `generate_random_string(10)`，`cron_source=Some(cron_name)`）；Phase 3 `insert_many(...).exec_with_returning(db)` 用 RETURNING 取回每行真实 `Model`，经纯函数 `pair_task_ids` 按写入顺序派生 `(真实 id, uuid, token)` 三元组（**切勿**用 `last_insert_id - (count-1)` 反推——PG 并发下 id 交错，见下方「内部机制」）；Phase 4 `JoinSet` 并发 `TaskManager::global().send_event` 发送 `TaskEvent{ task_id (u64 cast), task_token, task_event_type }`；Phase 5 收集结果，失败入 `failed_task_ids` 批量 `delete_many`（`Id.is_in`，仅删本批真实 id），构建 `CrontabResult`；Phase 6 `insert_many crontab_result`（单轮）；`info_span!` target `crontab`。
...
### crontab_task 用 RETURNING 取回真实 id

`crontab_task`（`task.rs`）批量 `insert_many(...).exec_with_returning(db)` 写入 N 条 task 记录，**用 `RETURNING` 拿回每行的真实 `Model`**（含 DB 实际分配的 `id`），再经纯函数 `pair_task_ids(&inserted)` 派生每个 agent 的派发三元组 `(真实 id, uuid, token)`，按下标顺序与 `uuids` 一一配对。

**为何不用 `last_insert_id` 反推区间**：`base_id = last_insert_id - (count-1)` 隐含「批次内 id 连续递增」假设，这在 PostgreSQL 下**不成立**。task 表主键是 `GENERATED BY DEFAULT AS IDENTITY`（CACHE 默认 1），其 `nextval` **按行原子获取**——并发 `insert_many`（多个 cron 同一 tick `JoinSet` 并发 fire、cron 与 `task.create`/`create_task_blocking` RPC 并发、cron 与 JS Worker 建任务并发）会让本批拿到的 id 交错（如本批得 `100/102/104`、他批得 `101/103/105`）。反推会把**属于别的批次的 id** 当作本批任务下发，导致 `task_id + uuid + token` 三元组错配：`upload_task_result`（`crates/ng-task/src/rpc/upload_task_result.rs:85-99`）按三元组严格匹配 DB → `Task validation failed`，结果被拒、`task.success` 长空、`relative_id`/`cron_source` 错配，且失败回滚会**删除其他并发批次的 task 行**。SQLite 因单条 INSERT 持库锁、rowid 单调连续而**不受影响**——这也是该竞态仅在 PostgreSQL 生产部署暴露的原因。

`RETURNING` 行顺序 == VALUES 写入顺序 == `task_models` 构建顺序 == `uuids` 顺序，故 `dispatch[i]` 精确对应 `uuids[i]`。`pair_task_ids` 为纯函数（无 DB/全局依赖），其单测直接用「不连续 id（`100/102/104`）」作输入，锁定「不依赖 id 连续性」这一修复核心。
...
- **切勿**用 `last_insert_id` 反推 `insert_many` 批次的连续 id 区间：PostgreSQL 的 IDENTITY 序列 `nextval` 按行原子获取，并发写入会让本批 id 交错（如 `100/102/104`），`base_id = last_id - (count-1)` 会把别的批次的 id 当作本批下发，导致 `task_id+uuid+token` 三元组错配。`crontab_task` 已改用 `insert_many(...).exec_with_returning(db)` 取回每行真实 id 按写入顺序配对（纯函数 `pair_task_ids`，`crates/ng-crontab/src/task.rs`）。任何新增的批量插入若需要回读 id，同样必须用 `RETURNING`，而非反推。
...
- **注意** Agent 类型 cron 在调度层是 fire-and-forget：`crontab_task` 仅依据 `send_event`（channel 入队）是否成功记录 success，不反映 agent 是否真正执行；`CrontabResult.success` 含义是「已入队」而非「已运行」（`send_event` 调用于 `crates/ng-crontab/src/task.rs:172`，其返回结果决定 `CrontabResult.success` 于 `:198`（true）/`:219`（false））。
